...
multilib_src_install() {
	...
	cp "${FILESDIR}/lua.pc" "${WORKDIR}/lua-c++.pc" || die
	sed -r -i \
		-e "/^INSTALL_INC=/s,(/include)$,\1/lua${SLOT}," \
		-e "s:^prefix= :prefix= ${EPREFIX}:" \
		-e "s:^V=.*:V= ${PATCH_PV}:" \
		-e "s:^R=.*:R= ${PV}:" \
		-e "s:/,lib,:/$(get_libdir):g" \
		-e "/^Libs:/s:( )(-llua)($| ):\1-l${LIBNAME}-c++\3:" \
		-e "/^includedir=/s:include$:include${INCLUDEDIR_SUFFIX}:" \
		"${WORKDIR}/lua-c++.pc" || die

	insinto "/usr/$(get_libdir)/pkgconfig"
	newins "${WORKDIR}/lua.pc" "lua${SLOT}.pc"
	newins "${WORKDIR}/lua-c++.pc" "lua${SLOT}-c++.pc"
	...
	dosym "lua${SLOT}.pc" "/usr/$(get_libdir)/pkgconfig/lua${SLOT/.}.pc"
	dosym "lua${SLOT}-c++.pc" "/usr/$(get_libdir)/pkgconfig/lua-${SLOT}-c++.pc"
	dosym "lua${SLOT}-c++.pc" "/usr/$(get_libdir)/pkgconfig/lua${SLOT/.}-c++.pc"
}
